...
export function Settings(props: SettingsProps) {
  ...
  return <section className="settings" aria-label="Agent Product settings">
    ...
    <div className="settings__agents">
      {props.agents.filter((agent) => isPhaseOneAgentProductId(agent.id)).map((agent) => <article key={agent.id} className="settings__agent-card">
        <div className="settings__agent-row">
          <div><h2><span aria-hidden="true" className={`settings__agent-state settings__agent-state--${displayState(agent)}`}>{statusText(agent, zh)}</span>{agent.name}</h2><p>{zh ? "版本" : "Version"}: {agent.version ?? "—"}</p></div>
          <div className="settings__agent-actions">
            <button type="button" aria-label={`${zh ? "导入其他" : "Import another"} ${agent.name} ${zh ? "路径" : "path"}`} onClick={() => void props.onPickExecutablePath(agent.id).then((path) => { if (path) props.onSaveExecutablePath(agent.id, path); })}>{zh ? "导入其他路径" : "Import another path"}</button>
            <button type="button" aria-label={`${agent.enabled === false ? (zh ? "启用" : "Enable") : (zh ? "停用" : "Disable")} ${agent.name}`} disabled={displayState(agent) === "absent"} onClick={() => props.onSetEnabled?.(agent.id, agent.enabled === false)}>{agent.enabled === false ? (zh ? "启用" : "Enable") : (zh ? "停用" : "Disable")}</button>
          </div>
        </div>
        <section aria-label={`${agent.name} catalog models`} className="settings__models"><h3>{zh ? "可用模型" : "Available models"}</h3>{agent.catalog.models.length ? <ul>{agent.catalog.models.map((model) => <li key={model}>{model}</li>)}</ul> : <p>{zh ? "未发现可用模型" : "No catalog models reported"}</p>}</section>
      </article>)}
    </div>
  </section>;
}
