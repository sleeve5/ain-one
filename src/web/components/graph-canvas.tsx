...
import { applyNodeChanges, Background, BaseEdge, ControlButton, Controls, EdgeLabelRenderer, getBezierPath, MarkerType, ReactFlow, SelectionMode, useReactFlow, type Connection, type Edge, type EdgeChange, type EdgeProps, type Node, type NodeChange, type ReactFlowInstance } from "@xyflow/react";
...
function RoutableEdge({ id, sourceX, sourceY, sourcePosition, targetX, targetY, targetPosition, markerEnd, label, style, selected, data }: EdgeProps<Edge<RoutableEdgeData>>) {
  ...
  const labelX = route?.x ?? defaultLabelX;
  const labelY = route?.y ?? defaultLabelY;
  const path = route ? smoothRoutedPath({ sourceX, sourceY, targetX, targetY, route }) : defaultPath;
...
export function smoothRoutedPath({ sourceX, sourceY, targetX, targetY, route }: { sourceX: number; sourceY: number; targetX: number; targetY: number; route: { x: number; y: number } }): string {
  const length = Math.hypot(targetX - sourceX, targetY - sourceY) || 1;
  const tangent = { x: (targetX - sourceX) / length, y: (targetY - sourceY) / length };
  const sourceSpan = Math.min(Math.hypot(route.x - sourceX, route.y - sourceY) / 3, 80);
  const targetSpan = Math.min(Math.hypot(targetX - route.x, targetY - route.y) / 3, 80);
  return `M ${sourceX},${sourceY} C ${sourceX + (route.x - sourceX) / 3},${sourceY + (route.y - sourceY) / 3} ${route.x - tangent.x * sourceSpan},${route.y - tangent.y * sourceSpan} ${route.x},${route.y} C ${route.x + tangent.x * targetSpan},${route.y + tangent.y * targetSpan} ${targetX + (route.x - targetX) / 3},${targetY + (route.y - targetY) / 3} ${targetX},${targetY}`;
}
...
export function GraphCanvas({ language = "en", api, graphId, agents = [], view = "editor", clearRequest = 0, onGraphSaved, onValidationChange }: GraphCanvasProps) {
  ...
  useEffect(() => {
    ...
    void api.getGraph(graphId).then(async (detail) => {
      ...
      requestAnimationFrame(() => flow.current?.fitView({ padding: 0.18, duration: 240 }));
  ...
  return <section className="graph-canvas" data-library-open={libraryOpen} data-inspector-open={inspectorOpen} style={{ "--graph-inspector-width": `${inspectorWidth}px` } as React.CSSProperties} data-testid="graph-canvas" aria-label="Graph Canvas">
    ...
    <div className="graph-workspace">
      <div className="graph-flow" data-testid="graph-flow"><ReactFlow nodes={flowNodes} edges={flowEdges} nodeTypes={nodeTypes} edgeTypes={edgeTypes} fitView fitViewOptions={{ padding: 0.18 }} minZoom={0.25} maxZoom={1.8} deleteKeyCode={null} nodesDraggable={interactionMode === "select"} nodesConnectable={interactionMode === "select"} elementsSelectable={interactionMode === "select"} selectionOnDrag={interactionMode === "select"} selectionMode={SelectionMode.Partial} panOnDrag={interactionMode === "pan"} onInit={(instance) => { flow.current = instance; requestAnimationFrame(() => instance.fitView({ padding: 0.18 })); }} onNodesChange={changeNodes} onNodesDelete={(nodes) => updateGraph((current) => removeSelection(current, new Set(nodes.map((node) => node.id)), new Set()))} onEdgesChange={(changes: EdgeChange[]) => { const selection = changes.filter((change): change is Extract<EdgeChange, { type: "select" }> => change.type === "select"); if (selection.length) { const next = new Set(selectedEdgeIdsRef.current); for (const change of selection) change.selected ? next.add(change.id) : next.delete(change.id); selectedEdgeIdsRef.current = next; setSelectedEdgeIds(next); } const removed = new Set(changes.filter((change) => change.type === "remove").map((change) => change.id)); if (removed.size) updateGraph((current) => removeSelection(current, new Set(), removed)); }} onConnect={(connection) => updateGraph((current) => connect(current, connection))} onReconnect={(_oldEdge, connection) => updateGraph((current) => reconnect(current, _oldEdge.id, connection))} edgesReconnectable onMoveEnd={(_event, viewport) => updateGraph((current) => ({ ...current, viewport }))}><Background gap={18} size={1} color="#c9d8e8" /><Controls showInteractive={false}><ControlButton title={interactionMode === "select" ? (zh ? "切换到拖动画布" : "Switch to pan mode") : (zh ? "切换到选择模式" : "Switch to select mode")} aria-label={interactionMode === "select" ? (zh ? "切换到拖动画布" : "Switch to pan mode") : (zh ? "切换到选择模式" : "Switch to select mode")} data-active={interactionMode === "pan"} onClick={() => setInteractionMode((current) => current === "select" ? "pan" : "select")}>{interactionMode === "select" ? <HandIcon /> : <PointerIcon />}</ControlButton></Controls></ReactFlow></div>
