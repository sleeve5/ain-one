...
function PromptEditor({ value, ports, label, unknownLabel, onChange }: { value: string; ports: GraphPort[]; label: string; unknownLabel: string; onChange(value: string): void }) {
  const parts = splitPrompt(value);
  return <div className="graph-prompt-editor" role="textbox" aria-label={label} contentEditable suppressContentEditableWarning onInput={(event) => onChange(serializePrompt(event.currentTarget))} onPaste={(event) => { event.preventDefault(); insertPlainText(event.clipboardData.getData("text/plain")); const editor = event.currentTarget; queueMicrotask(() => onChange(serializePrompt(editor))); }}>{parts.map((part, index) => part.type === "text" ? <span key={index}>{part.value}</span> : <span key={index} className="graph-prompt-token" contentEditable={false} data-parameter-id={part.id}>{ports.find((port) => port.id === part.id)?.name ?? unknownLabel}</span>)}</div>;
}
...
export function connect(graph: GraphProject, connection: Connection): GraphProject {
  ...
  if (target?.type === "agent") {
    ...
    if (targetPort === "__new_input" || occupied) {
      ...
      const name = feedback ? `Loop ${current.filter((port) => port.kind === "feedback").length + 1}` : `Input ${current.filter((port) => port.kind !== "feedback").length + 1}`;
