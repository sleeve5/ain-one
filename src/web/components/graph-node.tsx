...
export const GRAPH_NODE_HEADER_HEIGHT = 56;
export const GRAPH_NODE_PORT_HEIGHT = 20;
export const GRAPH_NODE_PORT_PADDING = 8;
...
export function GraphNodeView({ data, selected }: NodeProps) {
  ...
  return <div className="graph-node" data-kind={value.kind} data-selected={selected || undefined} data-status={value.status}>
    <header style={{ height: GRAPH_NODE_HEADER_HEIGHT, minHeight: GRAPH_NODE_HEADER_HEIGHT }}><span className="graph-node__icon" aria-hidden="true">{icon(value.kind)}</span><span className="graph-node__copy"><strong>{value.label}</strong>{value.subtitle && <small>{value.subtitle}</small>}</span></header>
